...
function resolveEmoteCommand(input: string): { type: 'action' | 'join', value: ParticipantActions | ParticipantClasses } | undefined {
    switch (input) {
...
export function prepareEmoteCommandListener(): void {
    eventBus.subscribe("chat_message", (data: any) => {
        ...
        if (emoteCmd.type === 'action') {
            ...
            if (playerPair) {
                playerPair.player.lastAction = emoteCmd.value as ParticipantActions;
            ...
            handlePlayerJoin(username, emoteCmd.value as ParticipantClasses);
        }
    });
} 